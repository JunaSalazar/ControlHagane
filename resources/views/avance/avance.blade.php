        ...
        <tr>
          ...
          <td>
            @if(!Auth::user()->hasRole('customer'))
                  ...
                  <form action="{{URL('avance/'. isset($avance) ?: '')}}" method="POST">
                  <a href="{{URL('avance/'. $r->avanceId.'/edit')}}" class="btn btn-success gradient glyphicon glyphicon-eye-edit"><span class="glyphicon glyphicon-edit"></span></a>
                  </form>

                  {{-- *************** TERMINA BOTÓN DE EDITAR DATOS*************** --}}

                  
                    @component('avance.editaravance')
                    @slot('editar')
                    <label>{{ $nombre }}</label>
                    @endslot
                    @endcomponent
                ...
                <td>
                  ...
                  {{--{!!Form::open(['route'=> ['avance.destroy', $r->avanceId], 'method' => 'DELETE'])!!}--}}
